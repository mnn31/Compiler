 ...
 *
 * The core idea (from slides): given a non-terminal on top of the
 * parsing stack and the current lookahead token, consult the 2-D parse
 * table TABLE[nonTerminal][token] to get the production to expand.
 * Terminals on the stack are matched (eaten) against the input.
 *
 * LL(1) Grammar (left-recursion removed via the A->betaA',
 ...
 *
 * FIRST sets (hand-computed following slides rules):
 ...
 * A'->alphaA'|epsilon transformation to the left-recursive expr and
 * term rules, (4) hand-computing FIRST and FOLLOW sets,
 * (5) constructing the predictive parse table, and (6) implementing
 * this table-driven parser so it produces the same AST nodes as the
...
public class LL1Parser
{
     ...
     * to apply.
     *
     * Table construction rule (from slides):
